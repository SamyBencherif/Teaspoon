...
//  Teaspoon-GL
//
//  Created by Samy Bencherif on 12/26/18.
//  Copyright © 2018 Samy Bencherif. All rights reserved.
//

import Foundation

func runTeaspoonProgram()
{

let TeaspoonJS = """

var binding = Binding.instantiate()
...
binding.func_print(resolve(args[0], src, scopeNames, scopeValues))
else
binding.func_print(jsString(resolve(args[0], src, scopeNames, scopeValues))) // process string literal or vars
return;
}
...
function test()
{
binding.func_print('plz')
}
"""

    evalJSWithBindings(src: TeaspoonJS)
